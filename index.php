<?php include_once("header.php") ;?>
<head>
    <meta name="description" content="La plateforme dédiée au partage de connaissances bucco-dentaires">
    <title>Dentiio | Accueil</title>
</head>

<!--HEADER-->
<div class='container pb-4'>
    <div class="row">
        <div class='col-md-6'>
            <div class="pt-md-5 pl-md-5">
                <h1 class="text-primary text-res pt-2 title-name">Dentiio</h1>
                <h1 class='text-res text-grey font-weight-light title-word animated bounceInLeft'>La référence en diagnostic dentaire<br><span class="title-word2">Car chaque cas clinique est unique</span></h1>
                <ul class="bloc-list mt-5">
                    <li class="text-list">
                        Découvrez des protocoles thérapeutiques
                    </li>
                    <li class="text-list mt-3">
                        Échangez avec vos confrères
                    </li>
                    <li class="text-list mt-3">
                        Partagez votre pratique
                    </li>
                </ul>
                <div class="mt-5 mb-3 text-center">
                    <a href="#atout" class="button bg-primary p-2 border-0 ml-0 border-radius50 text-white">Explorer Dentiio <i class="fa fa-arrow-right pl-2"></i></a>
                </div>
            </div>
        </div>
        <div class='col-md-6 illusbloc'>
            <img class='illus' alt="Dentiio application screnn" src="img/illusordi.png"/>
        </div>
    </div>
</div>
<!--EXPLICATION PICTO-->
<p id="atout" class="mb-5"></p>
<div class='container bck-grey picto-bloc px-3 pt-2 pb-md-5 pb-2'>
    <div class="row pb-md-5">
        <div class="col-12 col-md-12 col-lg-12 p-3 mt-2">
            <h2 class="text-grey text-uppercase text-center title2">nos atouts</h2>
        </div>
        <div class='item33 text-center'>
            <img class='picto' alt="validate your diagnosis" src="img/picto/tooth.png"/>
            <h2 class='picto-title text-grey'>Validez votre diagnostic</h2>
            <p class='desc text-center text-black-50 px-md-4'>Dialoguez avec d’autres praticiens lors de vos prises de décisions grâce à notre plateforme.</p>
        </div>
        <div class='item33 text-center'>
            <hr class='picto-hr'/>
            <img class='picto' alt="Target your priority: the patient" src="img/picto/dentist.png"/>
            <h2 class='picto-title text-grey'>Ciblez votre priorité: le patient</h2>
            <p class='desc text-center text-black-50 px-md-4'>Choisissez la meilleure thérapeutique à adopter pour votre patient et diversifiez vos méthodes en consultant des cas cliniques réels et certifiés. </p>
        </div>
        <div class='item33 text-center'>
            <hr class='picto-hr'/>
            <img class='picto' alt="Become an actor in your community" src="img/picto/teamwork.png"/>
            <h2 class='picto-title text-grey'>Devenez acteur de votre communauté</h2>
            <p class='desc text-center text-black-50 px-md-4'>Démarquez vous en publiant vos cas, et faites partie d’une communauté d'échanges et d’entraide.</p>
        </div>
    </div>
</div>

<!--Valeurs Desktop-->
<div class='container bg-secondary d-none d-md-block'>
    <div class="row">
        <div class="col-0 col-md-1 col-lg-1"></div>
        <div class="col-12 col-md-10 col-lg-10">
            <img class="mw-100 z-10 mt-n5 mb-n5 w-100 position-relative" alt="dentist team" src="img/photo/dentist-photo.png">
            <div class="caption bg-white09 z-10">
                <h2 class="text-primary text-uppercase text-left p-2 title2">notre solution</h2>
                <p class="text-grey p-2 desc">
                    Nous sommes engagés à accompagner les chirurgiens dentistes aussi bien dans le processus de diagnostic que dans la prise de décisions.<br><br>
                    Grâce à la plateforme en ligne Dentiio, échanger entre confrères n’a jamais été aussi simple et confortable,
                    tout en gardant une source d’information fiable et en créant une entraide au sein de la communauté dentaire.
                </p>
            </div>
        </div>
        <div class="col-0 col-md-1 col-lg-1"></div>
    </div>
    </div>
</div>

<!--Valeurs Mobile-->
<div class='container d-md-none bg-primary py-5 px-2'>
    <div class="row">
        <div class="col-12">
            <h2 class="text-white text-uppercase text-center title2">notre solution</h2>
            <p class="text-white text-center pt-2 desc">
                Nous sommes engagés à accompagner les chirurgiens dentistes aussi bien dans le processus de diagnostic que dans la prise de décisions.<br><br>
                Grâce à la plateforme en ligne Dentiio, échanger entre confrères n’a jamais été aussi simple et confortable,
                tout en gardant une source d’information fiable et en créant une entraide au sein de la communauté dentaire.
            </p>
        </div>
    </div>
</div>

<!--HOW-->
<h2 class="text-grey text-uppercase title2 text-center mt-md-5 pt-5">Le principe</h2>
<div class='container bg-white pt-md-5 mt-md-3'>
    <div class="row pb-5 px-md-4">
        <div class="col-md-6">
            <ol class="gradient-list">
                <li class="li-one"><h3>Recherchez les pathologies, techniques ou traitements qui vous intéresse.</h3></li>
                <p class="text-grey pt-5">Lorem ipsum dolor sit amet, consectetur adipiscing elit. Vivamus sed volutpat urna.</p>
            </ol>
        </div>
        <div class="col-md-6">
            <img class="w-100 border-primary border" alt="Home of application" src="img/home.png">
        </div>
    </div>
    <div class="row pt-5 pb-5 px-md-4 bck-grey">
        <div class="col-md-6 order-md-last">
            <ol class="gradient-list">
                <li class="li-two"><h3>Consultez et commentez les cas déjà publiés par d’autres praticiens.</h3></li>
                <p class="text-grey pt-5">Lorem ipsum dolor sit amet, consectetur adipiscing elit. Vivamus sed volutpat urna.</p>
                <!--
                <li></li>
                <li>Débattez en direct sur des questions posées par vos confrères.</li>-->
        </div>
        <div class="col-md-6 order-md-first">
            <img class="w-100 border-primary border" alt="Share your diagnosis with Dentiio application" src="img/home.png">
        </div>
    </div>
    <div class="row pt-5 px-md-4 pb-5">
        <div class="col-md-6">
            <ol class="gradient-list">
                <li class="li-three"><h3>Publiez vos cas dentaires les plus pertinents.</h3></li>
                <p class="text-grey pt-5">Lorem ipsum dolor sit amet, consectetur adipiscing elit. Vivamus sed volutpat urna.</p>
        </div>
        <div class="col-md-6">
            <img class="w-100 border-primary border" alt="Chat on Dentiio application" src="img/home.png">
        </div>
    </div>
</div>

<!--OUR SOLUTION-->
<hr class='picto-hr'/>
<div class='container mt-4'>
    <div class="row">
        <div class='col-md-6 illusbloc'>
            <img class='illus illusordi' alt="Dentists treating a tooth" src="img/illus.png"/>
        </div>
        <div class='col-md-6'>
            <div class='pt-4 text-center'>
                <div class="mt-3 pt-md-5">
                    <h2 class="text-primary text-uppercase text-center pt-md-5 title2">Formulaire</h2>
                </div>
                <p class="text-black-50 pt-3 px-md-5 mt-3  text-center">Notre plateforme est en développement. Elle sera prochainement disponible sur web, mobile & tablette.<br><br></p>
                <p class="text-grey pt-3 mt-1 px-md-5 text-center font-weight-bold ft-s2">Inscrivez-vous à la communauté Dentiio et recevez toutes nos informations exclusives.</p>
                <a href="https://docs.google.com/forms/d/e/1FAIpQLSfS_jq9N_vC1eYy48ZX8I3hqyO7R4be91k5Vi_2BUymXkM-eg/viewform?usp=sf_link" target='_blank' rel="noopener noreferrer" class="nav-item button buttonhome border-0"><span class="gradient"></span>Répondre au questionnaire</a>
            </div>
        </div>
    </div>
</div>
<?php include_once("footer.php") ;?>




